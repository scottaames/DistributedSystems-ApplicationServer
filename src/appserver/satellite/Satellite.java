...
public class Satellite extends Thread {
    ...
    public void run() {
        ...
        // ---------------------------------------------------------------
        try {
            Socket serverConnection = new Socket(serverInfo.getHost(), serverInfo.getPort());
            Message registerMsg = new Message(REGISTER_SATELLITE, satelliteInfo);
            ObjectOutputStream writeToNet = new ObjectOutputStream(serverConnection.getOutputStream());
            System.out.println("[Satellite.run] Sending message to server to register satellite: " + satelliteInfo.getName() + "...");
            writeToNet.writeObject(registerMsg);
        } catch (IOException e) {
            System.err.println("[Satellite.run] Satellite registration message failed.");
        }
        ...
        public void run() {
            ...
            try {
                ...
                switch (message.getType()) {
                    case JOB_REQUEST:
                        System.out.println("[SatelliteThread.run] Received job Request.");
                        try {
                            ...
                            System.out.println("\n[SatelliteThread.run] *COMPLETED JOB* Processed job - result: " + (int) result);
